...
def train_cnn():
	...
	max_document_length = 200  # max([len(x.split(' ')) for x in x_raw])
	logging.info('The maximum length of all sentences: {}'.format(max_document_length))
	vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length, 10, tokenizer_fn=data_helper.chinese_tokenizer)
	# 保存词对应索引
	...
	vocab_processor.save('data/vocab.bin')
	...
	with open('./labels2.json', 'w') as outfile:
	...
	with graph.as_default():
		...
		with sess.as_default():
			...
			out_dir = os.path.abspath(os.path.join(os.path.curdir, "model/trained_model_" + timestamp))
			...
			for test_batch in test_batches:
				...
				total_y_true.extend([np.argmax(i) for i in y_test_batch])
				total_y_pred.extend(list(y_pred))

			test_accuracy = float(total_test_correct) / len(y_test)
			scores = precision_recall_fscore_support(total_y_true, total_y_pred, average='micro')
			logging.critical('Accuracy on test set is {} based on the best model {}'.format(test_accuracy, path))
			logging.critical('score:{}'.format(scores))
			logging.critical('The training is complete')


if __name__ == '__main__':
	# python train.py ./data/data.csv.zip ./parameters.json
